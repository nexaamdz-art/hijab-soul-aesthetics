...
const PRODUCTS_KEY = "hijab_soul_products_v7";
...
export function getStoredProducts(): AdminProduct[] {
  ...
  try {
    ...
    // Filter out any products that use non-user-provided images
    const valid = parsed.filter(
      (p) => p.image.startsWith("/images/products/") || p.image.startsWith("/images/categories/"),
    );
    ...
    if (hasNew || valid.length !== parsed.length) {
      localStorage.setItem(PRODUCTS_KEY, JSON.stringify(valid));
    }
    return valid;
  } catch {
    return INITIAL_PRODUCTS;
