...
export function SiteHeader({ onMenuClick }: { onMenuClick?: () => void }) {
  ...
  return (
    <header className="w-full">
      {/* Top Brown Bar */}
      <div className="paper-dark bg-header text-header-foreground">
        <div className="mx-auto flex w-full max-w-[1400px] items-center gap-4 px-4 py-2 sm:px-6 lg:gap-8 lg:py-3">
      ...
      {/* Nav & Mobile Search (on Cream Paper Background to match Hero Artwork) */}
      <div className="paper-cream">
        <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6">
          ...
          >
            <div className="torn-paper relative w-full bg-paper px-5 py-2 shadow-sm">
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="إبحث عن ما تحبين .."
          ...
          <nav aria-label="التصنيفات" className="hidden lg:block">
            <ul className="flex items-center justify-start gap-7 pb-1.5 pt-1.5 xl:gap-10">
              {navItems.map((item) => (
                <li key={item.label}>
                  <Link
                    to={item.to}
                    className="block border-b-2 border-transparent pb-0.5 text-[13px] font-bold text-ink/90 transition-colors hover:text-burgundy"
                    activeProps={{ className: "border-burgundy text-burgundy" }}
