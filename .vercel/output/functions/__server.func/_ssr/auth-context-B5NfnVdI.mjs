...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-context-B5NfnVdI.js
...
var ADMIN_EMAILS = ["nexa.am.dz@gmail.com"];
...
function AuthProvider({ children }) {
	...
	const signUpWithEmail = async ({ email, password, firstName, lastName }) => {
		...
		if (accounts.findIndex((a) => a.email.toLowerCase() === normalizedEmail) >= 0) return { error: /* @__PURE__ */ new Error("هذا البريد الإلكتروني مسجل بالفعل! يرجى تسجيل الدخول بدلاً من ذلك.") };
	...
	const signInWithEmail = async ({ email, password }) => {
		...
		if (!found) {
			if (isSupabaseConfigured) try {
				...
				if (res.error) return { error: /* @__PURE__ */ new Error("بيانات الدخول غير صحيحة أو الحساب غير موجود.") };
			} catch (err) {
				return { error: err instanceof Error ? err : new Error(String(err)) };
			}
			return { error: /* @__PURE__ */ new Error("لم يتم العثور على حساب بهذا البريد الإلكتروني. يرجى إنشاء حساب جديد أولاً.") };
		}
		if (found.passwordHash !== password) return { error: /* @__PURE__ */ new Error("كلمة المرور غير صحيحة. يرجى التأكد من كلمة المرور والمحاولة مجدداً.") };
	...
	const signInWithGoogle = async (options) => {
		if (isSupabaseConfigured && !options?.email) try {
		...
		const emailToUse = (options?.email || "nexa.am.dz@gmail.com").trim();
		const isOwner = isAdminEmail(emailToUse);
		const nameToUse = options?.fullName?.trim() || (isOwner ? "نورة (المديرة العامة)" : emailToUse.split("@")[0] || "مستخدم Google");
		const nameParts = nameToUse.split(" ");
		const firstName = nameParts[0] || "مستخدم";
		const lastName = nameParts.slice(1).join(" ") || "Google";
		const avatar = options?.avatarUrl || (isOwner ? "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200" : void 0);
		const googleUser = {
		...
		const accounts = getLocalAccounts();
		const existingIndex = accounts.findIndex((a) => a.email.toLowerCase() === emailToUse.toLowerCase());
		if (existingIndex >= 0) accounts[existingIndex] = {
