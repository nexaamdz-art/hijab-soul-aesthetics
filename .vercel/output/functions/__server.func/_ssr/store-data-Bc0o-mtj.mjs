...
//#region node_modules/.nitro/vite/services/ssr/assets/store-data-Bc0o-mtj.js
...
var CURRENT_DATA_VERSION = "v16";
var PRODUCTS_KEY = `hijab_soul_products_${CURRENT_DATA_VERSION}`;
var ORDERS_KEY = `hijab_soul_orders_${CURRENT_DATA_VERSION}`;
var CONVERSATIONS_KEY = `hijab_soul_conversations_${CURRENT_DATA_VERSION}`;
var CATEGORIES_KEY = `hijab_soul_categories_${CURRENT_DATA_VERSION}`;
var HERO_BANNER_KEY = `hijab_soul_hero_banner_${CURRENT_DATA_VERSION}`;
...
function getProductFallbackImage(category) {
	...
	return DEFAULT_CATEGORY_IMAGES[category === "hijab-robe" ? "abayas" : category] || "/images/categories/abayas.jpg";
}
function cleanupLegacyStorage() {
	...
	try {
		for (let i = 0; i < localStorage.length; i++) {
			...
			if (key && key.startsWith("hijab_soul_") && !key.endsWith(`_${CURRENT_DATA_VERSION}`)) localStorage.removeItem(key);
		}
	} catch {}
}
...
function getStoredCategories() {
	...
	try {
		...
		const synchronized = parsed.filter((c) => c.id !== "hijab-robe").map((cat) => {
		...
		for (const initCat of INITIAL_CATEGORIES) if (!existingIds.has(initCat.id)) synchronized.push(initCat);
...
function getStoredProducts() {
	...
	try {
		...
		const synchronized = parsed.map((prod) => {
			...
			const validImage = prod.image && prod.image.trim() !== "" ? prod.image : getProductFallbackImage(normalizedCat);
			return {
				...prod,
		...
		for (const initProd of INITIAL_PRODUCTS) if (!existingIds.has(initProd.id)) synchronized.push(initProd);
...
function useStoreData() {
	...
	const [orders, setOrders] = (0, import_react.useState)(INITIAL_ORDERS);
	const [conversations, setConversations] = (0, import_react.useState)(INITIAL_CONVERSATIONS);
	const [categories, setCategories] = (0, import_react.useState)(INITIAL_CATEGORIES);
	const [heroBanner, setHeroBanner] = (0, import_react.useState)(null);
	...
	return {
		...
		addOrder: (0, import_react.useCallback)((newOrder) => {
			setOrders((prev) => {
		...
		sendMessage: (0, import_react.useCallback)((conversationId, text, sender = "admin", imageUrl, productAttachment) => {
			const newMessage = {
				...
				imageAttachment: imageUrl || void 0,
				productAttachment: productAttachment || void 0
			};
...
//#endregion
export { useStoreData as a, hijab_soul_mark_default as i, compressImageFile as n, getProductFallbackImage as r, CURATED_IMAGE_PRESETS as t };
