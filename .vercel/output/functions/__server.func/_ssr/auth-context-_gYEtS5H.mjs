...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-context-_gYEtS5H.js
...
function AuthProvider({ children }) {
	...
	const signInWithGoogle = async (options) => {
		...
		const nameToUse = options?.fullName?.trim() || (isOwner ? "المدير العام" : emailToUse.split("@")[0] || "مستخدم Google");
		const nameParts = nameToUse.split(" ");
		const firstName = nameParts[0] || "مستخدم";
		const lastName = nameParts.slice(1).join(" ") || "Google";
		const avatar = options?.avatarUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(firstName)}&backgroundColor=2B2119&textColor=FFFFFF`;
		const googleUser = {
