...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-context-CcZM0j3x.js
...
function AuthProvider({ children }) {
	...
	const signUpWithEmail = async ({ email, password, firstName, lastName }) => {
		...
		try {
			...
			if (res.error && (res.error.message?.includes("rate limit") || res.error.message?.includes("confirmation email") || res.error.code === "over_email_send_rate_limit")) {
				const fallbackUser = accountToUser(accountRecord);
				setUser(fallbackUser);
				localStorage.setItem(LOCAL_STORAGE_USER_KEY, JSON.stringify(fallbackUser));
				return {
